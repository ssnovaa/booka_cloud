...
class RewardsController extends Controller
{
    ...
    public function admobSsv(Request $r)
    {
        ...
        return DB::transaction(function () use ($userId, $adUnitId, $minutesToAdd, $transactionId, $nonce, $r) {
            ...
            if ((int) $event->user_id !== $userId) {
                Log::notice('admobSsv: event user mismatch', [
            ...
            // Начисляем минуты (переводим в секунды и обновляем seconds_left)
            $this->addMinutes($userId, $minutesToAdd);
    ...
    /**
     * Начислить пользователю прослушиваемое время.
     *
     * ВАЖНО: живём только в секундах.
     *
     * Требования к listen_credits:
     *  - UNIQUE индекс на (user_id)
     *  - колонки: user_id INT, seconds_left INT NULL|DEFAULT 0, created_at, updated_at
     *
     * Поведение:
     *  - если записи нет — создаём с seconds_left = minutes * 60;
     *  - если seconds_left NULL — считаем, что было 0;
     *  - увеличиваем seconds_left атомарно.
     */
    private function addMinutes(int $userId, int $minutes): void
    {
        ...
        DB::transaction(function () use ($userId, $addSec) {
            ...
            // Берём текущие секунды; если NULL — считаем, что 0
